...
export default function BudgetBiteAI() {
  ...
  const [stock, setStock] = useState(""); // 余っている食材用
  const [userRequest, setUserRequest] = useState("1週間分の献立と買い物リストを教えて。");
  const [history, setHistory] = useState<{ id: number, name: string, price: number, date: string }[]>([]);
  ...
  const resetData = () => {
    if (confirm("データをリセットして、今月の予算を¥25,000に戻しますか？")) {
      setBudget(25000);
      setAiResponse("");
      setHistory([]);
      setStock("");
  ...
  const askGemini = async () => {
    ...
    try {
      ...
      最後にだいちゃんへの応援メッセージも添えて、マークダウン形式で回答してね。`;
      
      const result = await model.generateContent(prompt);
      setAiResponse(result.response.text());
    } catch (error) {
      setAiResponse("エラーが発生しました。APIキーを確認してください。");
    }
    setLoading(false);
  };
